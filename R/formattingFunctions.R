...
#'
#' @param dataFrame a data.frame object where the column names are prefixes that will be prepended to each identifier in that column. Please specify either this argument or filePath. Supported prefixes can be found with getPrefixesFromAnalytes()
#' @param filePath a string containing the file path to a csv that will be read and converted to RaMP data input format. Column names should be prefixes that will be prepended to each identifier in that column.
...
#' dataFrame <- file.path(dir,"ExampleInputFileName.csv")
#'
...
#' RaMPInput <- createRaMPInput(filePath = 'createRaMPInput_test_data.csv')
#'
#' getPathwayFromAnalyte(RaMPInput)
#'}
#' @export
