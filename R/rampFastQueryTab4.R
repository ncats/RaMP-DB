...
#  print(list_metabolite)
  ...
  if(length(grep("RAMP_C",df1$rampId)!=0)){
   ...
   if(nrow(df_c2) == 0){
    ...
    mdf_cfin <- mdf_cfin[,c("analyte1","sourceId","IDtype","commonName")]
  ...
  if(length(grep("RAMP_G",df1$rampId))!=0){
    ...
    query_g <- paste0("select * from catalyzed where rampGeneId in (",
                      g_id,");")
   ...
   # Now merge it all:
    ...
    mdf_gfin <- mdf_gfin[,c("analyte1","sourceId","IDtype","commonName")]
    colnames(mdf_gfin) <- c("Input_Metabolite","Gene_sourceId","Gene_IDtype",
        "Gene_CommonName")

    # Collapse source ids:
    mdf_gfin$temp <- paste(mdf_gfin[,"Input_Metabolite"],mdf_gfin[,"Gene_CommonName"])
    tempout <- data.frame(Input_Metabolite=NA,Gene_CommonName=NA,Gene_sourceIds=NA)
    mdf_gfin2=c()
...
##' 
##' @param analytes string or data.frame contains given analyte
##' @param con a connection object returned from the function connectToRaMP()
##' @return a data.frame that contains search results
#rampFastMulCata <- function(analytes,con) {
#  now <- proc.time()
...
#  list_metabolite <- paste(list_metabolite,collapse = ",")
#  print(list_metabolite)
#  query1 <- paste0("select Synonym as analyte1,rampId,geneOrCompound as type1 from analyteanalytes where Synonym in (",
#                   list_metabolite,");")
#  df1<- DBI::dbGetQuery(con,query1)
#  print(df1$rampId)
#  df_c <- df_g <- NULL
