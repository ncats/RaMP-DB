...
The latter line cannot be modiiefd, other than changes to the parameters inside the function (e.g., values for dbname, username, conpass, and host).  For example, if your password is different than "", you would set conpass="mypassword".  This line needs be run only once, every time the package is loaded.
...
for(i in 1:nrow(myanalytes)) {
  char.length <- nchar(myanalytes$sourceCompoundIDs[i])
  if(char.length >= cutoff) {
    new.string <- substr(myanalytes$sourceCompoundIDs[i], 1, cutoff)
    new.string <- paste0(new.string, "...")
    myanalytes$sourceCompoundIDs[i] <- new.string
  }
}
```
...
formattable(metabolite.classes)
```
...
formattable(classy_fire_classes)
```
