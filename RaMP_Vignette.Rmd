...
RaMP supports queries and enrichment analyses.  Supported queries are:
1) Retrieve Analytes From Input Pathway(s)
2) Retrieve Pathways From Input Analyte(s)
3) Retrieve Metabolites from Metabolite Ontologies
4) Retrieve Ontologies from Input Metabolites
5) Retrieve Analytes Involved in the Same Reaction
7) Retrieve Chemical Classes from Input Metabolites
8) Retrieve Chemical Properties from Input Metabolites

Supported enrichment analyses are:
9) Perform Pathway Enrichment
10) Perform Chemical Enrichment

Once installed, first load the package:
```{r}
library(RaMP)
library(formattable) # for prettier tables in vignette
...
```{r}
formattable(display_results(myanalytes))
```

To retrieve information from multiple pathways, input a vector of pathway names:
```{r}
myanalytes <- getAnalyteFromPathway(pathway=c("De Novo Triacylglycerol Biosynthesis", "sphingolipid metabolism"))
```
...
formattable(display_results(pathwaydfids))
```
...
formattable(display_results(new.metabolites))
```
...
```{r, eval=FALSE}
# INCORPORATE COLE'S function
formattable(display_results(ontology.table))
```
...
metabolites.of.interest <- c("ensembl:ENSG00000135679", "hmdb:HMDB0000064",
        "hmdb:HMDB0000148", "ensembl:ENSG00000141510")
new.ontologies <- RaMP::getOntoFromMeta(analytes = metabolites.of.interest)
formattable(display_results(new.ontologies))
```
...
new.transcripts <- rampFastCata(analytes = metabolites.of.interest)

formattable(display_results(new.transcripts))
...
formattable(display_results(metabolite.classes))
```
...
formattable(display_results(chemical.data))
```
...
pathwaydfids <- getPathwayFromAnalyte(c("ensembl:ENSG00000135679", "hmdb:HMDB0000064",
        "hmdb:HMDB0000148", "ensembl:ENSG00000141510"))
fisher.results <- RaMP::runCombinedFisherTest(pathwaydf = pathwaydfids)
```
...
formattable(display_results(filtered.fisher.results, NA))
```
...
formattable(display_results(RaMP::write_FishersResults(fishResults=clusters), NA))
```

To view clustered pathway results:

```{r}
pathwayResultsPlot(filtered.fisher.results, text_size = 10, perc_analyte_overlap = 0.2, 
	min_pathway_tocluster = 2, perc_pathway_overlap = 0.2)
```
...
chemical.enrichment <- chemicalClassEnrichment(classData = chemical.classes)
classy_fire_classes <- chemical.enrichment$ClassyFire_class
formattable(display_results(classy_fire_classes, NA))
```
