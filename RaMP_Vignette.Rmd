...
```{r}
  ...
  print(paste("IDs to be prepended for genes:",paste(df1[,1],collapse=", ")))
```
...
```{r}
print(paste("Number of Unique Pathways Returned for All Analytes:", length(unique(pathwaydfids$pathwaysourceId))))
lapply(unique(pathwaydfids$commonName), function(x) {
        (paste("Number of Unique Pathways Returned for",x,":",
                length(unique(pathwaydfids[which(pathwaydfids$commonName==x),]$pathwaysourceId))))})
```
...
```{r, results='hide'}
fisher.results <- RaMP::runCombinedFisherTest(analytes = c(
...
```{r}
clusters <- RaMP::findCluster(fisher.results,
...
```{r, fig.height = 10}
pathwayResultsPlot(fisher.results, text_size = 10, perc_analyte_overlap = 0.2, 
